...
namespace ReturneeManager.Application.Features.Dashboards.Queries.GetData
{
    ...
    internal class GetDashboardDataQueryHandler : IRequestHandler<GetDashboardDataQuery, Result<DashboardDataResponse>>
    {
        ...
        public async Task<Result<DashboardDataResponse>> Handle(GetDashboardDataQuery query, CancellationToken cancellationToken)
        {
            var response = new DashboardDataResponse
            {
                ...
                PersonCount = await _unitOfWork.Repository<Person>().Entities.CountAsync(cancellationToken),
            ...
            double[] personsFigure = new double[13];
            ...
            for (int i = 1; i <= 12; i++)
            {
                ...
                personsFigure[i - 1] = await _unitOfWork.Repository<Person>().Entities.Where(x => x.CreatedOn >= filterStartDate && x.CreatedOn <= filterEndDate).CountAsync(cancellationToken);
            ...
            response.DataEnterBarChart.Add(new ChartSeries { Name = _localizer["Beneficiaries"], Data = personsFigure });
